...
(function (phoenix) {
    ...
    var feedbackContent = (function (_super) {
        ...
        feedbackContent.prototype.html2Canvas = function (documentWidth, docoumentheight) {
            ...
            html2canvas($('body'), {
                onrendered: function (canvas) {
                    ...
                    $('#fb-overview').show();
                    setTimeout(function () {
                        $('#fb-screenshot').html('').append('<a href="' + img + '" target="_blank" ><img id="imagee" class="fb-screenshot fb-screenshot-border fb-screenshot-fadeIn" src="' + img + '" /></a>');
        ...
        feedbackContent.prototype.submitFeedback = function () {
            ...
            var imgSource = document.getElementById('imagee').src ;
            $.ajax({
                url: this.url,
                dataType: 'json',
                type: 'POST',
                data: { description: $('.description').val() ,type:$('#type').val(),image:imgSource  },
